...
struct CollectionScanner<C> where C: Collection {
    ...
    mutating func scan(count: Int) -> SubSequence? {
        guard !atEnd else {
            return nil
        }
...
// TODO: This operates on a COPY of the Scanner
struct CollectionScannerIterator<C>: IteratorProtocol where C: Collection, C.Element: Equatable {
    typealias Scanner = CollectionScanner<C>
    var scanner: Scanner
    let block: (inout Scanner) -> Scanner.SubSequence?
    mutating func next() -> Scanner.SubSequence? {
        if scanner.atEnd {
            return nil
        }
